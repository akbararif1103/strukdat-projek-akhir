    ...
    do
    {
        ...
        { // Tambahkan case untuk menu menghapus produk
            if (!isEmpty(groceryStore))
            {
                ...
                deleteProductFromTree(groceryStore, productCode); // Memasukkan ke dalam pohon biner setelah dihapus
            }
            else
            {
                cout << "Product list is empty." << endl;
...
void addProduct(GroceryStore &store, const Product &product)
{
    ...
    for (int i = 0; i < store.deletedProductCount; ++i)
    {
        if (strcmp(store.deletedProducts[i].code, product.code) == 0)
        {
...
void deleteProductFromTree(GroceryStore &store, const char *productCode)
{
    int index = hashFunction(productCode);
    for (int i = 0; i < store.productCount; ++i)
    {
